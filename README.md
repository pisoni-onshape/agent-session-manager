...
If you regularly use more than one AI coding tool, your conversation history ends up fragmented: Copilot CLI sessions live under `~/.copilot/`, Cursor transcripts under `~/.cursor/`, VS Code Copilot transcripts in `~/Library/Application Support/Code/`, Claude Code (CLI & VS Code) sessions under `~/.claude/projects/`, and Claude Desktop titles under `~/Library/Application Support/Claude/`. Agent Session Manager indexes all of them into a single local catalog and lets you search, filter, star, rename, and resume any session in seconds.
